# rowplux
This plugin allows cordova application to use the biosignalsplux java api 
...
### isEnabled
This call will check if Bluetooth is enabled it will also check if geolocation permission is granted a request to turn on permission may be prompted. A message will return if permission is denied by user this is applicable for android marshmallow and later
...
This call will return the version of a connected biosignalsplux device the description of the device the number of channels the PID The FW version and the HW version
...
This call will return the state of connected biosignalsplux device the api returns a long value
...
The stopError callback contains the information.
    ...
    note for second and subsequent startRecording stopRecording cycles the api throws an error reciever not registered requires investigating to see what the effect of this is.
